...
| 12 | 2026-09-16 | Packaging against the Submission Guide | **BLOCKED** | Two blocking, both text. **§3.1's 95.2% any-hit@3 is the floor 0.00-0.15 row of the tuning sweep; at the 0.40 floor that ships it is 92.7%** — the same mistake as review 8's 65.5%-versus-64.0%, in a different measure, and it had propagated to D-21 and the Stage 5 log. And the PRD's §2 note, which said the problem statement must be rewritten in the author's own words, was **removed without doing what it asked** and without disclosure, while the Project Instructions list the problem statement among the things a model must not produce. Ten non-blocking: R-10's impact abbreviated away in Table 10 and R-06 called "most likely" without support; Table 11 naming the wrong log, omitting the kill-switch release condition and one notify case; the `-dirty` provenance marker firing on any redirected output because `git status --porcelain` counts untracked files; a review-4 condition (retrieval guard >=0.90) never applied and logged as applied in review 5; four stale counts; and several loose PRD attributions. Independently reproduced: every Figure 1 figure and contrast ratio, all six recovery handlers now executed, 509 tests, 91.67% coverage, and that the PRD kept its 4 September requirements |
